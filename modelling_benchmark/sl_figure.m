...
%% evaporation data
evapo_kgs = zeros(inp.nn1,time_step);
for i=1:inp.nn1
...
yyaxis left
    ...
    axis([-0.1 time_day(end) -0.5 2000])
yyaxis right
    ...
    axis([-0.1 time_day(end) -0.5 100])

    %title('ead profile');

    %% -------------  sub 2 ET for each node  --------------
    a.sub2=subplot('position'...
         ,[fig_pos.left,fig_pos.bottom-0.3,...
          fig_pos.length,fig_pos.height]);
    a.plot2=plot(evapo_mmday(:,nt),...
